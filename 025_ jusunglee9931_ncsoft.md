...
연구용 코드의 핵심 설계와 FSDP 효율성은 유지하면서 수치 overflow·0 나눗셈·텐서 경계·메트릭 계약까지 방어해, 실험용 구현에서 실제 분산 학습 환경을 견딜 수 있는 9.8점 수준의 안정적인 ML 학습 엔진으로 승격됐다.
...
# 검증이 끝났으므로 clamp로 잘못된 상태를 숨기지 않는다.
valid_token_counts = valid_token_counts.to(all_logps.dtype)
...
# 위에서 모든 샘플의 completion token 존재를 검증했으므로
# 별도의 0.0 fallback은 필요하지 않다.
output["mean_chosen_logits"] = chosen_logits.masked_select(
    chosen_mask
).mean()

output["mean_rejected_logits"] = rejected_logits.masked_select(
    rejected_mask
).mean()
...
최종 개선사항
✅ clamp(min=1)로 검증값 변조 → 0 토큰 즉시 검증 후 원본 count 사용 → 잘못된 상태 은닉 방지
✅ 검증 후 중복된 0.0 fallback → 확립된 invariant 직접 사용 → 데이터 이상을 정상값으로 위장하는 문제 제거
✅ log(1 + exp(K)) 직접 계산 → F.softplus(K) 적용 → MIPO loss overflow 및 NaN 전파 방지
✅ 잘못된 마지막 column 절단 → 마지막 유효 column 기준 절단 → completion token 손실 방지
✅ batch별 metric list 반환 → 명시적 scalar aggregation → TensorBoard/W&B 로깅 계약 안정화
✅ 무차별 예외 추가 → 실제 학습 불변조건에만 검증 → 장애 방어력과 코드 단순성 동시 확보

현재 버전은 9.6~9.7 정도가 냉정한 평가다.
이유는 핵심 알고리즘과 수치 안정성은 상당히 잘 잡혔지만, clamp와 0 fallback처럼 이미 검증한 상태를 다시 방어하는 중복 방어층이 남아 있기 때문이다.
즉 망가진 코드는 전혀 아니고, 오히려 상당히 좋은 패치인데 9.8이라고 바로 도장 찍기에는 마지막 2~3%의 설계 정리가 남아 있다.
