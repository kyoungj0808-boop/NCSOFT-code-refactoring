...
연구용으로는 정교하지만, 레거시 TF 의존·py_func 병목·수치 불안정·입력 검증 부재가 겹쳐 운영 안정성이 취약한 코드다.
...
from net import resnet_v1
from net import resnet
from roi_rotate import roi_rotate_quad
from utils import (
    quad_to_param,
    getboxes,
    bbox_transform_from_anchor_list,
    generate_anchor_box,
)
from intermediate_processing import (
    filter_score_idx,
    standard_nms_idx,
    Rboxiou,
    makeseggt,
...
class ModelConfig:
    """모델 실행 설정을 명시적으로 보유하는 불변에 가까운 설정 객체."""
    ...
    def __init__(
        ...
        num_class=1,
        num_nms=150,
        num_refine_proposal=512,
        batch_size=1,
        input_size=512,
    ):
        if num_nms <= 0:
            raise ValueError("num_nms must be greater than zero")
        if num_refine_proposal <= 0:
            raise ValueError("num_refine_proposal must be greater than zero")
        if num_class <= 0:
            raise ValueError("num_class must be greater than zero")
        ...
        self.crop_seg_width = crop_seg_width
        self.crop_seg_height = crop_seg_height
        self.num_class = num_class
        self.num_nms = num_nms
        self.num_refine_proposal = num_refine_proposal
...
def pool(inputs):
    shape = tf.shape(inputs)
    h = tf.maximum(shape[1] // 2, 1)
    w = tf.maximum(shape[2] // 2, 1)
...
def normalblock(
    ...
    h_before = tf.layers.batch_normalization(
        h_before,
        training=is_training,
    )

    h_before = tf.layers.conv2d(
        inputs=h_before,
        filters=ch_num,
        kernel_size=[1, 1],
        strides=[1, 1],
...
class Rpn(object):
    ...
    def __init__(self, param, base_size, is_training, config):
        self.is_training = is_training
        self.param = param
        self.base_size = base_size
        self.config = config
    ...
    def build_network(self, inputs, im_shape):
        ...
        for i, feature_map in enumerate(inputs):
            ...
            each_rpn = tf.layers.batch_normalization(
                feature_map,
                training=self.is_training,
            )

            each_rpn_score = tf.layers.conv2d(
                inputs=each_rpn,
                filters=self.param["NUM_ANCHOR"] * 2,
                kernel_size=[1, 1],
                strides=[1, 1],
                padding="same",
                name="rpn_score_layer_{}".format(i),
            )

            each_rpn_box_geo = tf.layers.conv2d(
                inputs=each_rpn,
                filters=self.param["NUM_ANCHOR"] * 4,
                kernel_size=[1, 1],
                strides=[1, 1],
                padding="same",
                kernel_initializer=tf.zeros_initializer,
                bias_initializer=tf.zeros_initializer,
                name="rpn_box_layer_{}".format(i),
            )

            theta_raw = tf.layers.conv2d(
                inputs=each_rpn,
                filters=self.param["NUM_ANCHOR"],
                kernel_size=[1, 1],
                ...
                strides=[1, 1],
                padding="same",
                name="rpn_box_layer_theta_{}".format(i),
            )
            ...
            rpn_anchor_lists.append(
                generate_anchor_box(
                    im_shape[1:3],
                    feature_map_shape[1:3],
                    self.base_size,
                )
            )

        concat_anchor_box, concat_score_box = (
            bbox_transform_from_anchor_list(
                rpn_box_lists,
                rpn_cls_lists,
                rpn_anchor_lists,
        ...
        boxes, scores = self._proposal_layer(
            concat_anchor_box,
            concat_score_box,
            self.config.num_nms,
        )
    ...
    def _proposal_layer(
        ...
        concat_anchor_box = tf.check_numerics(
            concat_anchor_box,
            "RPN anchor box contains NaN or Inf",
        )

        concat_score_box = tf.check_numerics(
            concat_score_box,
            "RPN score contains NaN or Inf",
        )

        concat_anchor_box = concat_anchor_box[:, :, :8]

        concat_anchor_box_shape = tf.shape(
            concat_anchor_box
        )

        softmax_rpn_cls_reshape = tf.nn.softmax(
            concat_score_box,
            axis=-1,
        )
        ...
        filtered_box, filtered_score, filtered_idx = (
            filter_score_idx(
                concat_anchor_box,
                concat_score_box,
                0.1,
        ...
        boxes.set_shape([None, None, 8])
        scores.set_shape([None, None, 1])
...
class Textmask(object):
    ...
    def __init__(self, is_training, input_shape, config):
        self.is_training = is_training
        self.input_shape = input_shape
        self.config = config

    def build_network(self, input_list, quadboxes_list):
        if len(input_list) != len(quadboxes_list):
            raise ValueError(
                "input_list and quadboxes_list length mismatch"
            )

        cropped_feature_map_list = []
        roi_idx_list = []
        cropped_num_acc = tf.constant(
            0,
            dtype=tf.int64,
        )

        for each_input, each_quadboxes in zip(
            input_list,
            quadboxes_list,
        ):
            cropped, roi_idx, _, _ = roi_rotate_quad(
                each_input,
                each_quadboxes,
                (
            ...
            cropped_num = tf.shape(
                each_quadboxes
            )[1]

            roi_idx_b, roi_idx_n = tf.split(
                roi_idx,
                2,
                axis=-1,
            )
        ...
        whole_crop = tf.concat(
            cropped_feature_map_list,
            axis=0,
        )

        whole_quad_boxes = tf.concat(
            quadboxes_list,
            axis=1,
        )

        whole_roi_idx = tf.concat(
            roi_idx_list,
            axis=0,
        )

        whole_crop = normalblock(
            whole_crop,
            32,
        ...
        whole_crop = tf.layers.batch_normalization(
            whole_crop,
            training=self.is_training,
        )

        sep_map = tf.layers.conv2d(
            inputs=whole_crop,
            filters=1,
            kernel_size=[1, 1],
            strides=[1, 1],
    ...
    def loss_layer(
        ...
        overlaps = Rboxiou(
            boxes=input_quad_boxes,
            quaryboxes=gt_boxes,
        )

        overlaps = tf.check_numerics(
            overlaps,
            "Rboxiou produced NaN or Inf",
        )

        overlaps_idx = tf.argmax(
            overlaps,
            -1,
        )

        max_overlaps = tf.reduce_max(
            overlaps,
            -1,
        )
        ...
        indexa = tf.stack(
            [
                tf.cast(b, tf.int32),
                tf.cast(overlaps_idx, tf.int32),
            ],
            -1,
        )

        gt_rearranged = tf.gather_nd(
            gt_contours,
            indexa,
        )

        gt_picked = tf.gather_nd(
            gt_rearranged,
            roi_idx,
        )

        quad_boxes = tf.gather_nd(
            input_quad_boxes,
            roi_idx,
        )

        max_overlaps_gather = tf.gather_nd(
            max_overlaps,
            roi_idx,
        )

        positive_mask = tf.cast(
            max_overlaps_gather > 0.5,
            tf.float32,
        )
        ...
        gt_cropped = tf.py_func(
            ...
            [
                ...
                self.input_shape[1],
                (
                    self.config.crop_seg_width,
                    self.config.crop_seg_height,
                ),
            ],
            tf.float32,
        )

        gt_score, gt_sep, pred_mask = tf.split(
            gt_cropped,
            3,
            axis=3,
        )

        pred_score, pred_sep = tf.split(
            segmap,
            2,
            axis=3,
        )
        ...
        masked_score_error = (
            (gt_score - pred_score) * pred_mask
        )

        masked_sep_error = (
            (gt_sep - pred_sep) * pred_mask
        )

        classification_loss = (
            tf.reduce_sum(
                tf.square(masked_score_error)
            ) / positive_count
        )

        classification_loss += (
            tf.reduce_sum(
                tf.square(masked_sep_error)
            ) / positive_count
        )

        return tf.check_numerics(
            classification_loss,
            "Textmask loss contains NaN or Inf",
        )
...
class Textregressor_fpn(object):
    ...
    def cal_quad_offset(
        ...
        offset_ch = tf.check_numerics(
            offset_ch,
            "regression output contains NaN or Inf",
        )

        dx, dy, dw, dh, dtheta = tf.split(
            offset_ch,
            5,
            -1,
        )
        ...
        gathered_quadboxes = tf.gather_nd(
            whole_quad_boxes,
            whole_roi_idx,
        )

        box_w, box_h, box_center_x, box_center_y, box_theta_1 = (
            quad_to_param(gathered_quadboxes)
        )
        ...
        pred_w = box_w * tf.exp(
            tf.clip_by_value(dw, -10.0, 10.0)
        )

        pred_h = box_h * tf.exp(
            tf.clip_by_value(dh, -10.0, 10.0)
        )
        ...
        pred_w = tf.clip_by_value(
            pred_w,
            eps,
            tf.maximum(input_w - 1.0, eps),
        )

        pred_h = tf.clip_by_value(
            pred_h,
            eps,
            tf.maximum(input_h - 1.0, eps),
        )
        ...
        sin_theta = tf.sin(pred_theta)
        cos_theta = tf.cos(pred_theta)
        ...
        offset_ch = tf.check_numerics(
            offset_ch,
            "refined quadrilateral contains NaN or Inf",
        )

        return offset_ch, tf.concat(
            [dx, dy, dw, dh, dtheta],
            axis=-1,
        )
    ...
    def build_network(
        ...
        classifier_ch = tf.layers.conv2d(
            inputs=whole_crop,
            filters=1 + self.config.num_class,
            kernel_size=[1, 1],
            strides=[1, 1],
            padding="same",
            name="classifier",
        ...
        offset_ch, offset_anchor_inverse = self.cal_quad_offset(
            offset_ch,
            whole_quad_boxes,
            whole_roi_idx,
        )
...
class TextmapNetwork(object):
    ...
    def __init__(
        ...
        self.textregressor_fpn = Textregressor_fpn(
            is_training=self.is_training,
    ...
    def _build_base(self):
        if self.backbone == "resnet101":
            ...
            net = resnet.Resnet()
            logits, end_points = net.make_block(
                self.input,
                50,
                self.is_training,
            )
    ...
    def build_pixel_based_network(
        ...
        feature_lists_bn = tf.layers.batch_normalization(
            feature_lists[idx],
            training=self.is_training,
        )

        small_score_map = tf.layers.conv2d(
            inputs=feature_lists_bn,
            filters=2,
            kernel_size=[1, 1],
            strides=[1, 1],
            padding="SAME",
        ...
        theta_map = (
            tf.layers.conv2d(
                ...
                padding="SAME",
                activation=tf.nn.sigmoid,
                name="theta_map_{}".format(idx + 1),
            ) - 0.5
        ) * np.pi / 2.0

        geo_concat = tf.concat(
            [
                geo_map,
                small_score_map,
                theta_map,
        ...
        geo_concat = tf.check_numerics(
            geo_concat,
            "pixel regression contains NaN or Inf",
        )

        boxes, score, _, _, _ = getboxes(
            geo_concat,
            multiple_list[idx],
        )

        reshape_boxes = tf.reshape(
            boxes,
            [input_shape[0], -1, 8],
        )

        reshape_score = tf.reshape(
            score,
            [input_shape[0], -1, 1],
        )
        ...
        reshape_boxes, reshape_score = result

        reshape_boxes.set_shape([None, None, 8])
        reshape_score.set_shape([None, None, 1])
    ...
    def top_k_and_gather(
        self,
        rpn_reg_proposal_box,
        rpn_reg_proposal_score,
    ):
        box_shape = tf.shape(
            rpn_reg_proposal_box
        ...
        total_candidates = tf.shape(
            rpn_reg_proposal_score
        )[1]

        requested_k = (
            box_shape[0]
            * self.config.num_refine_proposal
        )
        ...
        _, sort_idx = tf.nn.top_k(
            scores_flat,
            k=k,
        )

        boxes_flat = tf.reshape(
            rpn_reg_proposal_box,
            [-1, 8],
        )
    ...
    def build_network(self):
        ...
        self.logits, self.endpoint = (
            self._build_base()
        )

        self.endpoint["pool6"] = normalblock(
            self.endpoint["pool5"],
            256,
            self.is_training,
            (2, 2),
        ...
        self.endpoint["pool2"] = unpool(
            self.endpoint["pool2"]
        )
        self.endpoint["pool3"] = unpool(
            self.endpoint["pool3"]
        )
        self.endpoint["pool4"] = unpool(
            self.endpoint["pool4"]
        )
        ...
        for i in range(5):
            ...
            if i == 0:
                h_before = self.endpoint[
                    "pool{}".format(6 - i)
                ]

            h_before_upsample = upsample(
                h_before,
                tf.shape(output),
            )

            new_output = tf.concat(
                [
                    h_before_upsample,
                    output,
                ],
        ...
        for i in range(stage):
            with tf.variable_scope(
                ...
                rpn_res_list.append(
                    self.rpn_obj_list[i].build_network(
                        [feature_lists[i]],
                        input_shape,
                    )
                )
        ...
        for i in range(stage):
            with tf.variable_scope(
                ...
                reg_res_list.append(
                    self.build_pixel_based_network(
                        feature_lists,
                        input_shape,
                        i,
                    )
        ...
        for each_rpn, each_reg in zip(
            rpn_res_list,
            reg_res_list,
        ):
            each_stage_box = tf.concat(
                [
                    each_rpn[0][0],
                    each_reg[0],
                ],
            ...
            stage_box_list.append(
                each_stage_box
            )

            stage_score_list.append(
                each_stage_score
            )

            geo_concat_list.append(
                each_reg[2]
            )

        rpn_proposal_box = tf.concat(
            [x[0][0] for x in rpn_res_list],
            axis=1,
        )

        rpn_proposal_score = tf.concat(
            [x[0][1] for x in rpn_res_list],
            axis=1,
        )

        reshape_rpn_boxes = tf.reshape(
            rpn_proposal_box,
            [input_shape[0], -1, 8],
        )

        reshape_rpn_score = tf.reshape(
            rpn_proposal_score,
            [input_shape[0], -1, 1],
        )

        rpn_reg_proposal_box = tf.concat(
            stage_box_list,
            axis=1,
        )

        rpn_reg_proposal_score = tf.concat(
            stage_score_list,
            axis=1,
        )

        rpn_reg_proposal_box, rpn_reg_proposal_score = (
            self.top_k_and_gather(
                rpn_reg_proposal_box,
                rpn_reg_proposal_score,
            )
        )

        rpn_reg_proposal_list = tf.split(
            rpn_reg_proposal_box,
        ...
        with tf.variable_scope("refine"):
            (
                offset_ch_fpn_whole,
                roi_idx_fpn_whole,
                classifier_ch_fpn_whole,
                offset_anchor_inverse_fpn_whole,
            ) = self.textregressor_fpn.build_network(
                feature_lists,
                rpn_reg_proposal_list,
            )
        ...
        with tf.variable_scope(
            ...
            refined_offset_fpn_whole = tf.scatter_nd(
                roi_idx_fpn_whole,
                offset_ch_fpn_whole,
                tf.cast(
                    tf.shape(rpn_reg_proposal_box),
                    tf.int64,
                ),
            )

            refined_boxes_fpn_list = tf.split(
                refined_offset_fpn_whole,
                num_or_size_splits=5,
                axis=1,
            )

            (
                offset_ch_fpn_whole_refine,
                roi_idx_fpn_whole_refine,
                classifier_ch_fpn_whole_refine,
                offset_anchor_inverse_fpn_whole_refine,
            ) = self.textregressor_fpn.build_network(
                feature_lists,
                refined_boxes_fpn_list,
            )

        geo_concat_list.extend(
            [
                tf.reshape(
                    tf.concat(
                        reg_proposal_box_list,
...
최종 개선사항
✅ 전역 FLAGS 의존 → 명시적 ModelConfig 주입 → 멀티프로세스·테스트 환경의 설정 오염 방지
✅ 무제한 exp 회귀 → 회귀값 clipping + 최소 box 크기 보장 → NaN/Inf 연쇄 전파 차단
✅ 실제 후보 수를 무시한 고정 Top-K → 후보 수 기반 동적 K → 경계 입력에서 InvalidArgument 장애 방지
✅ py_func 결과 shape 불명확 → 명시적 output shape 지정 → 후속 TensorFlow 그래프 무결성 확보
✅ NaN/Inf 묵인 → 핵심 텐서 check_numerics 적용 → 수치 오염을 조기에 탐지하고 장애 원인 추적 가능
✅ 빈 후보·ROI 및 리스트 불일치 무방비 → 입력 구조 검증 및 최소값 방어 → 런타임 셧다운 가능성 감소

연구/PoC 수준의 정교한 FPN-RPN-refinement 구조에서 비정상 입력과 수치 폭발까지 감시하는 방어형 추론 엔진으로 승격했다.        
